...
export const VideoStudio: React.FC<VideoStudioProps> = ({ user }) => {
  ...
  const refreshRequests = () => {
      ...
      // Admins see pending, Users see all their own
  ...
  const handleRejectRequest = (req: MarketingRequest) => {
      const reason = window.prompt("Please provide a reason for rejection:");
      if (reason === null) return; // Cancelled

      const rejectedReq: MarketingRequest = {
          ...req,
          status: 'rejected',
          completedDate: new Date().toISOString(),
          rejectionReason: reason || 'No specific reason provided.'
      };
      storageService.updateMarketingRequest(rejectedReq);
      refreshRequests();
  };
  ...
  const handleCompleteRequest = () => {
      ...
      const completedReq: MarketingRequest = {
          ...
          completedDate: new Date().toISOString(),
          outputUrl: outputUrl,
          prompt: prompt, // Save edited prompt
          aspectRatio: aspectRatio as any // Save edited aspect ratio
      };
                              ...
                              <div>
                                  {req.status === 'completed' ? (
                                      ...
                                      <div className="flex flex-col items-end">
                                          ...
                                          {req.rejectionReason && <span className="text-[10px] text-red-500 mt-1">{req.rejectionReason}</span>}
                                      </div>
                                  ) : isAdmin && viewMode === 'requests' ? (
                                      <div className="flex gap-2">
                                          <button 
                                            onClick={() => handleRejectRequest(req)}
                                            className="px-3 py-2 bg-white border border-red-200 text-red-600 text-xs font-bold rounded hover:bg-red-50 transition-colors"
                                          >
                                              Reject
                                          </button>
                                          <button 
                ...
                <div className="bg-white dark:bg-slate-900 rounded-xl border border-slate-200 dark:border-slate-800 p-6 shadow-sm space-y-6">
                    ...
                    <div>
                        <label className="block text-xs font-bold text-slate-500 uppercase mb-2">
                            ...
                            {activeRequest && <span className="text-emerald-600 ml-2 text-[10px] normal-case bg-emerald-50 px-1.5 py-0.5 rounded border border-emerald-100">Editable</span>}
                        </label>
                        <textarea 
                            value={prompt}
